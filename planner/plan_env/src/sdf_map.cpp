...
void SDFMap::initMap(double x_size,double y_size,double z_size,double resolution_, Eigen::Vector3d org,double inflate_values)
{
  ...
  origin   = Eigen::Vector3d(-y_size / 2.0, -y_size / 2.0, -ground_z_);//org;//Eigen::Vector3d(-y_size / 2.0, -y_size / 2.0, ground_z_);
...
int SDFMap::setCacheOccupancy(Eigen::Vector3d pos, int occ)
{
  if (occ != 1 && occ != 0)
  {
    std::cout << "setCacheOccupancy occ value error!" << std::endl;
    return INVALID_IDX;
  }
  ...
  for (int x = 0; x < grid_size(0); ++x)
    for (int y = 0; y < grid_size(1); ++y)
      for (int z = 0; z < grid_size(2); ++z)
      {
        ...
        if(getInflateOccupancy(pos)>0)
        {
          std::cout << "[getmap]coodiates ="<< pos <<std::endl;
        }
